...
class AtariDQN(nn.Module):
    def __init__(self, input_shape, fc_size_list, activation, lr, loss_func):
...
# make_env from CleanRL implementation
...
def main():
    ...
    online_qnet = AtariDQN(obs_shape, fc_size_list=[512, n_actions], activation=nn.ReLU(), lr=INIT_LR, loss_func=nn.MSELoss()).to(device)
    target_qnet = AtariDQN(obs_shape, fc_size_list=[512, n_actions], activation=nn.ReLU(), lr=INIT_LR, loss_func=nn.MSELoss()).to(device)
    target_qnet.load_state_dict(online_qnet.state_dict())
    exp_replay = ExperienceReplay(BUFFER_SIZE, obs_shape, n_actions)
    ...
    for epoch in tqdm(range(N_EPOCH)):
        ...
        for i in range(N_STEPS):
            ...
            if np.random.uniform(0,1) < epsilon:
                ...
                a = torch.argmax(online_qnet(torch.from_numpy(np.array(s)).to(device))).item()
